...
const COMMON_5L_NAMES = new Set([
    // quick guardrail; model also avoids names
    'MASON', 'DYLAN', 'JAMES', 'SARAH', 'EMILY', 'LAURA', 'KAREN', 'KEVIN', 'BRIAN', 'DAVID', 'KYLIE', 'JULIA', 'ALICE'
])
const looksLikeName = (w) => COMMON_5L_NAMES.has(w)

// crude plural heuristic: words ending with S (but allow SS/US/IS/OS endings)
const likelyPlural = (w) => w.endsWith('S') && !/(SS|US|IS|OS)$/.test(w)
...
export const wordleGenerateNow = onRequest(
    { region: REGION, secrets: [OPENAI_API_KEY, ADMIN_API_KEY], cors: false },
    async (req, res) => {
        try {
            ...
            const todayId = dateParam && /^\d{4}-\d{2}-\d{2}$/.test(dateParam)
                ? dateParam
                : toDateId(new Date())

            const result = await runGenerationFor(todayId)
            res.status(200).json({ ok: true, todayId, ...result })
        } catch (e) {
