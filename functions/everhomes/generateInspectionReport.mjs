...
export const generateInspectionReport = onRequest(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (inspectorEmail) targets.push({ email: inspectorEmail, isAdmin: false });

            const emailResults = await Promise.allSettled(
                targets.map(t => resend.emails.send({
            ...
            if (adminSend.status === 'rejected' || adminSend.value?.error) {
                const reason = adminSend.reason?.message ?? adminSend.value?.error?.message ?? 'Unknown';
...
async function buildPDF({ propertyAddress, inspectionDate, inspectorName, rooms, photoAssets, schema, docTitle, sigAssets, reportSubtype }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Draw a pill badge (e.g. "SDA")
        function drawPill(x, y, text, textColor, bgColor) {
            const w = doc.widthOfString(text, { fontSize: 6 }) + 8;
            doc.roundedRect(x, y - 5, w, 9, 2).fill(bgColor);
            doc.font('Helvetica-Bold').fontSize(6).fillColor(textColor).text(text, x + 4, y - 3.5, { lineBreak: false });
            return x + w + 5;
        }

        // Section label
        function sectionLabel(text, y) {
            doc.font('Helvetica-Bold').fontSize(7).fillColor('#94A3B8').text(text, MARGIN, y);
            return y + 13;
        }
        ...
        doc.font('Helvetica-Bold').fontSize(20).fillColor('#1E293B')
            .text(propertyAddress || 'Unknown Property', MARGIN, 112, { width: CONTENT_W });
        ...
        doc.font('Helvetica-Bold').fontSize(7).fillColor('#94A3B8').text('ROOM OVERVIEW', MARGIN, STATS_Y - 12);
        doc.rect(MARGIN, STATS_Y, CONTENT_W, 62).fillAndStroke('#F8FAFC', '#E2E8F0');
        statItems.forEach((s, i) => {
            const x = MARGIN + i * STAT_W;
            doc.font('Helvetica-Bold').fontSize(20).fillColor(s.color)
                .text(String(s.val), x, STATS_Y + 8, { width: STAT_W, align: 'center' });
            doc.font('Helvetica').fontSize(7).fillColor('#94A3B8')
                .text(s.label, x, STATS_Y + 40, { width: STAT_W, align: 'center' });
        });

        // Item-level stats
        const ITEM_STATS_Y = STATS_Y + 78;
        ...
        doc.font('Helvetica-Bold').fontSize(7).fillColor('#94A3B8').text('CHECKLIST ITEMS', MARGIN, ITEM_STATS_Y - 12);
        doc.rect(MARGIN, ITEM_STATS_Y, CONTENT_W, 62).fillAndStroke('#F8FAFC', '#E2E8F0');
        iStatItems.forEach((s, i) => {
            const x = MARGIN + i * STAT_W;
            doc.font('Helvetica-Bold').fontSize(20).fillColor(s.color)
                .text(String(s.val), x, ITEM_STATS_Y + 8, { width: STAT_W, align: 'center' });
            doc.font('Helvetica').fontSize(7).fillColor('#94A3B8')
                .text(s.label, x, ITEM_STATS_Y + 40, { width: STAT_W, align: 'center' });
        });

        // OOA rooms
        const ooaRooms = rooms.filter(r => r.isOOA);
        if (ooaRooms.length) {
            doc.font('Helvetica-BoldOblique').fontSize(9).fillColor('#7C3AED')
                .text(`OOA Rooms: ${ooaRooms.map(r => r.label).join(', ')}`, MARGIN, ITEM_STATS_Y + 80);
        }
        ...
        if (flagged.length) {
            ...
            doc.font('Helvetica-Bold').fontSize(7).fillColor('#94A3B8').text('FLAGGED ROOMS', MARGIN, fy);
            fy += 12;
            ...
            for (const room of flagged) {
                ...
                doc.font('Helvetica-Bold').fontSize(9).fillColor('#1E293B')
                    .text(room.label, MARGIN + 14, fy, { width: CONTENT_W * 0.55, lineBreak: false });
                doc.font('Helvetica').fontSize(8).fillColor(sc)
                    .text(STATUS_META[room.status]?.label ?? '', MARGIN + 14 + CONTENT_W * 0.55, fy, { lineBreak: false });
                if (room.notes?.trim()) {
                    doc.font('Helvetica-Oblique').fontSize(8).fillColor('#64748B')
                        .text(room.notes.trim(), MARGIN + 14, fy + 11, { width: CONTENT_W - 14 });
                    fy = doc.y + 6;
                } else {
                    fy += 18;
                }
        ...
        for (const room of rooms) {
            ...
            if (room.isOOA) {
                doc.fillColor('#F43F5E').text('· OOA', tagX, tagY, { lineBreak: false });
                tagX += doc.widthOfString('· OOA') + 12;
            }
            if (room.isEnsuite) {
                doc.fillColor('#0891B2').text('· ENSUITE', tagX, tagY, { lineBreak: false });
            }
            ...
            const roomItemStats = { ok: 0, attention: 0, issue: 0, na: 0, unchecked: 0 };
            ...
            barItems.forEach((b, i) => {
                ...
                doc.font('Helvetica-Bold').fontSize(8).fillColor(b.color)
                    .text(String(b.val), bix, y + 3, { width: bItemW * 0.4, align: 'right', lineBreak: false });
                doc.font('Helvetica').fontSize(7).fillColor('#94A3B8')
                    .text(` ${b.label}`, bix + bItemW * 0.4, y + 4, { width: bItemW * 0.6, lineBreak: false });
            });
            y += BAR_H + 10;
            ...
            for (const group of groups) {
                ...
                doc.moveTo(MARGIN, y - 2).lineTo(PAGE_W - MARGIN, y - 2)
                ...
                for (let i = 0; i < group.items.length; i++) {
                    ...
                    if (isInput) {
                        ...
                        doc.rect(MARGIN + 4, y + 5, 6, 6).fillAndStroke('#EFF6FF', '#BFDBFE');
                        ...
                        const valW = 72;
                        doc.roundedRect(PAGE_W - MARGIN - valW, y + 2, valW, ITEM_ROW_H - 4, 2)
                            .fill('#EFF6FF');
                        ...
                        doc.font(item.sda ? 'Helvetica-Bold' : 'Helvetica').fontSize(8.5)
                ...
                for (let i = 0; i < roomPhotos.length; i += 2) {
                    ...
                    for (let j = 0; j < pair.length; j++) {
                        ...
                        try {
                            ...
                            doc.image(ph.buffer, x, y, { fit: [COL_W, rowImgH], align: 'center', valign: 'top' });
                        } catch { /* skip failed embed */ }
                    }
                    ...
                    doc.moveTo(MARGIN, y + rowImgH + captionBlockH + (ROW_GAP / 2))
            ...
            if (!room.notes?.trim() && !roomPhotos.length && statusItems.length === 0) {
                doc.font('Helvetica-Oblique').fontSize(10).fillColor('#CBD5E1')
                    .text('No checklist items, notes, or photos recorded for this room.', MARGIN, y);
            }
        }
        ...
        for (const group of groups) {
            for (const item of group.items) {
                ...
                if (status === 'issue' || status === 'attention') {
                    flaggedItems.push({ roomLabel: room.label, itemLabel: item.label, status, sda: !!item.sda });
                }
            }
        }
    }
 ...
 <div style="background:linear-gradient(135deg,#7C3AED 0%,#A855F7 100%);padding:0;">
  ...
  <div style="padding:28px 32px 24px; mso-line-height-rule:exactly;">
    ...
    ${flagged.length ? `
      ...
      </table>` : `<p style="color:#10B981;font-weight:600;font-size:13px;margin:0 0 20px;">&#10003; No rooms flagged with issues or attention items.</p>`}
...
function computeStats(rooms) {
    return {
        ...
        unchecked: rooms.filter(r => !r.status || r.status === 'unchecked').length,
    };
}
