...
### Journal retirement and Luna migration

The owner canceled journal key creation and retired the web journal. `/personal/journal` now shows an explicit retirement notice without mounting chat stores, message controls or API callers. Its route is readable without sign-in, and navigation labels identify the retirement. Stored journal data was not deleted. The unused `handleJournalChat` backend implementation was removed; Cloud Functions inventory contains no journal function and its former Sydney callable URL returns HTTP 404. The unrelated `/ai-journal` Discord-assistant overview is a separate feature.

Wordle daily/manual/unlimited generation now uses `gpt-5.6-luna` through the Responses API with strict JSON schema, bounded output, medium reasoning and `store: false`. Incomplete or malformed output is rejected before existing word filtering. Stored model metadata uses the same model constant. Connections generation and independent review also use Luna, retaining their quality checks, quotas and overwrite protections. The build passed 129 tests and schema validation; tests exercise outgoing requests and malformed/incomplete Responses results without paid provider calls.
