...
async function copyAndFilterWords() {
    ...
    const sourcePath = path.join(rootDir, 'node_modules', 'word-list-json', 'data', 'words.json');
    const altSourcePath = path.join(rootDir, 'node_modules', 'word-list-json', 'words.json');
    
    let wordsPath = fs.existsSync(sourcePath) ? sourcePath : altSourcePath;
    
    if (!fs.existsSync(wordsPath)) {
        console.error(`Could not find words.json at ${sourcePath} or ${altSourcePath}`);
        return;
    }
    ...
    // Output format: {"words": [...]}
    fs.writeFileSync(targetPath, JSON.stringify({ words: filteredWords }), 'utf8');
    console.log(`Successfully wrote ${filteredWords.length} words to ${targetPath}`);
}

async function copyCountries() {
    console.log('Processing countries.json...');
    const sourcePath = path.join(rootDir, 'node_modules', 'world-countries', 'countries.json');
    ...
    // Direct copy for countries (as requested)
    fs.copyFileSync(sourcePath, targetPath);
    console.log(`Successfully copied countries.json to ${targetPath}`);
}
