<template>
  <div
    class="flex flex-col items-center relative rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border backdrop-blur-md overflow-hidden group"
    :style="{ backgroundColor: cardBackground, borderColor: borderColor }"
  >
    <!-- Glow ring on hover -->
    <div
      class="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition duration-300 pointer-events-none"
    ...
    <!-- Name -->
    <p
      class="text-sm font-semibold text-center text-white/90 mb-2 relative z-10"
    >
      {{ name }}
    </p>
    ...
    <!-- Progress Bar -->
    <div
      class="w-full h-2 bg-black/30 rounded-full overflow-hidden relative z-10"
    >
      <div
        class="h-full rounded-full animate-glowPulse transition-all duration-1000"
...
const progressGradient = computed(
  () =>
    `linear-gradient(to right, ${props.baseColor}, ${darkenHex(props.baseColor, 0.2)})`,
);
