// @/features/topheroes/stores/useTopHeroesAdminStore.js
  ...
  getters: {
    ...
    systemHealth: (state) => {
      ...
      health.push({
        ...
        status:
          state.events.upcoming.length > 2
            ? "healthy"
            : state.events.upcoming.length > 0
              ? "warning"
              : "critical",
      ...
      const engagementScore =
        (state.analytics.userMetrics.activeUsers /
          Math.max(state.analytics.userMetrics.totalUsers, 1)) *
        100;
      health.push({
  ...
  actions: {
    ...
    applyDarkMode() {
      ...
      localStorage.setItem(
        "th-admin-dark-mode",
        JSON.stringify(this.settings.isDark),
      );
    },
