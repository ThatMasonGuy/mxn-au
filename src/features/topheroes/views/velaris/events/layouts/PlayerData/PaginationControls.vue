<template>
    <div class="flex flex-col md:flex-row justify-between items-center mt-4 gap-4">
        ...
        <div class="flex gap-2 items-center">
            <span class="text-sm text-primary-foreground mr-2">Items per page:</span>
            <select :value="itemsPerPage" @change="$emit('changeItemsPerPage', Number($event.target.value))"
                class="px-2 py-1 border rounded-md text-sm bg-background">
        ...
        <div class="flex gap-2">
            <button @click="$emit('changePage', Math.max(1, page - 1))" :disabled="page === 1"
                ...
                :class="page === 1 ? 'cursor-not-allowed text-muted-foreground bg-muted-background' : 'hover:bg-primary-background bg-background/80'">
                Previous
            </button>
            ...
            <button @click="$emit('changePage', Math.min(Math.ceil(totalItems / itemsPerPage), page + 1))"
                ...
                class="px-3 py-1 border rounded-md text-sm"
                :class="page === Math.ceil(totalItems / itemsPerPage) || totalItems === 0 ? 'cursor-not-allowed text-muted-foreground bg-muted-background' : 'hover:bg-primary-background bg-background/80'">
                Next
            </button>
        </div>
    </div>
