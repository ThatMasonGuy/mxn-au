<!-- @/features/topheroes/views/admin/TH_AdminCastleData.vue -->
<template>
  <div class="min-h-screen bg-gray-50 text-gray-800 p-6">
    ...
    <div class="shadow-2xl rounded-lg p-6 border">
          ...
          >
            <div class="flex items-center space-x-6">
              ...
              <div class="w-20 text-sm text-gray-600">
                {{ getPreviewValue(row, "woodStone") }}k
              </div>
              <div class="w-16 text-sm text-gray-600">
                {{ getPreviewValue(row, "rubies") }}k
              </div>
              <div class="w-16 text-sm text-gray-600">
                {{ getPreviewValue(row, "food") }}k
              </div>
              <div class="w-48 text-sm text-gray-500 truncate">
                {{ condensedBuildings(row) }}
              </div>
            </div>
          </AccordionTrigger>

          <AccordionContent
            class="px-6 py-4 border-t border-gray-200 space-y-4"
          >
            <div class="flex items-center space-x-2">
            ...
            <div class="grid grid-cols-4 gap-4">
              <div
                v-for="rarity in ['Rare', 'Epic', 'Legendary']"
                :key="rarity"
              >
                ...
                <div class="space-y-2">
                  ...
                  <Input
                    type="number"
                    v-model="row[`woodStone${rarity}`]"
                    class="w-full"
                  />
                  <Label>Rubies</Label>
                  <Input
                    type="number"
                    v-model="row[`rubies${rarity}`]"
                    class="w-full"
                  />
                  <Label>Food</Label>
                  <Input
                    type="number"
                    v-model="row[`food${rarity}`]"
                    class="w-full"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div
                  v-for="(building, bIdx) in row.requiredBuildings"
                  :key="bIdx"
                  class="flex space-x-2 items-center"
                >
                  <Select
                    :items="buildingOptions"
                    v-model="row.requiredBuildings[bIdx]"
                    class="w-full"
                  />
                  ...
                  <Popover>
                    ...
                    <PopoverContent class="p-4 space-y-2">
                      ...
                      <Input
                        type="number"
                        v-model="building.wood"
                        class="w-full"
                      />
                      <Label>Stone</Label>
                      <Input
                        type="number"
                        v-model="building.stone"
                        class="w-full"
                      />
                      <Label>Rubies</Label>
                      <Input
                        type="number"
                        v-model="building.rubies"
                        class="w-full"
                      />
                      ...
                      <Input
                        type="number"
                        v-model="building.days"
                        class="w-full"
                      />
                      <Label>Hours</Label>
                      <Input
                        type="number"
                        v-model="building.hours"
                        class="w-full"
                      />
                      <Label>Minutes</Label>
                      <Input
                        type="number"
                        v-model="building.minutes"
                        class="w-full"
                      />
                    </PopoverContent>
                  </Popover>
      ...
      <div class="mt-6 flex items-center space-x-4">
        <Button variant="outline" class="shadow-sm" @click="addRow"
          >Add Row</Button
        >
        <Button
          variant="default"
...
const buildingOptions = [
  { label: "Research Hall", value: "Research Hall" },
...
async function loadDataFromFirestore() {
  try {
    ...
    tableData.value = results.length
      ? results
...
function condensedBuildings(row) {
  return row.requiredBuildings.length
    ? row.requiredBuildings.map((b) => b.name).join(", ")
    : "—";
}
