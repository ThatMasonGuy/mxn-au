// @/features/topheroes/services/firestorePlayerData.js
import { preparePlayerEventData } from "@/shared/utils/prepPlayerEventData";
import { shouldSavePlayer } from "@/shared/utils/cleanNumber";
...
const deepSanitize = (obj, path = "") => {
  ...
  if (Array.isArray(obj)) {
    return obj
      .map((item, index) => deepSanitize(item, `${path}[${index}]`))
  ...
  if (typeof obj === "object") {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value === undefined) {
        console.warn(
          `[DEEP_SANITIZE] Removing undefined field at "${currentPath}"`,
        );
        continue;
      }
...
export const savePlayerToFirestore = async (player, currentEvent) => {
  ...
  const hasGvGScores =
    dayFields.some((f) => Number(player?.[f]) > 0) ||
    Number(player?.overallScore) > 0;
  ...
  const eventRef = doc(
    firestore,
    `topheroes/velaris/members/${player.id}/events/${eventId}`,
  );
  ...
  const eventChangeDate = currentEvent.endDate?.toDate
    ? currentEvent.endDate.toDate()
    : new Date();
  const memberUpdatedAt = memberData.updatedAt?.toDate
    ? memberData.updatedAt.toDate()
    : new Date(0);
  ...
  [...scoreFields, ...extraEventFields].forEach((field) => {
    const oldVal =
      existingEventData[field] === undefined ? null : existingEventData[field];
    const newVal =
      playerEventData[field] === undefined ? null : playerEventData[field];
  ...
  const historyEntry = {
    ...
    changes: Object.keys(cleanChanges).length
      ? cleanChanges
      : { forceMigration: true },
  };
  ...
  const existingHistory = Array.isArray(memberData.history)
    ? deepSanitize(memberData.history)
    : [];
  ...
  const validateNoUndefined = (obj, objName, path = "") => {
    ...
    if (Array.isArray(obj)) {
      ...
      Object.entries(obj).forEach(([key, value]) => {
        ...
        if (value === undefined) {
          throw new Error(
            `Found undefined value in ${objName} at path: ${currentPath}`,
          );
        }
