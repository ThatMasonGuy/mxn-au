...
<template>
  <div>
    ...
    <div class="mb-6 flex flex-wrap items-center gap-3">
      <div
        class="inline-flex items-center gap-2 rounded-full bg-th-success/15 px-4 py-2 text-sm text-th-success"
      >
        <span class="h-2 w-2 rounded-full bg-th-success animate-pulse"></span>
        {{ store.dashboardStats.totalGuides }} Active Guides
      </div>
      <div
        class="inline-flex items-center gap-2 rounded-full bg-th-primary/15 px-4 py-2 text-sm text-th-primary"
      >
        <BookOpen class="h-4 w-4" />
        {{ store.dashboardStats.totalHeroes }} Heroes
      </div>
      <div
        v-if="store.dashboardStats.activeEvents > 0"
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
      <!-- Total Guides Card -->
      <div
        class="th-stat-card bg-gradient-to-br from-th-primary/10 to-th-primary/5 border-th-primary/20"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-th-primary/5 to-transparent"
        ></div>
        <div class="relative p-2 h-full flex flex-col">
          <div class="flex items-center justify-between flex-1">
            <div class="flex-1">
              <div class="text-sm font-medium text-th-primary mb-1">
                Total Guides
              </div>
              <div class="text-2xl font-bold text-foreground mb-1">
                {{ store.dashboardStats.totalGuides }}
              </div>
            ...
            <div
              class="h-12 w-12 rounded-full flex items-center justify-center bg-th-primary/15"
            >
          ...
          <div v-if="guideGrowthTrend" class="mt-3 flex items-center gap-1">
            ...
            <span class="text-xs text-th-success font-medium">{{
              guideGrowthTrend.value
            }}</span>
      ...
      <!-- Monthly Views Card -->
      <div
        class="th-stat-card bg-gradient-to-br from-th-secondary/10 to-th-secondary/5 border-th-secondary/20"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-th-secondary/5 to-transparent"
        ></div>
        <div class="relative p-2 h-full flex flex-col">
          <div class="flex items-center justify-between flex-1">
            <div class="flex-1">
              <div class="text-sm font-medium text-th-secondary mb-1">
                Monthly Views
              </div>
              <div class="text-2xl font-bold text-foreground mb-1">
                {{ store.formatNumber(store.dashboardStats.monthlyViews) }}
              </div>
              <div class="text-xs text-foreground/60">
            ...
            <div
              class="h-12 w-12 rounded-full flex items-center justify-center bg-th-secondary/15"
            >
      ...
      <!-- Active Events Card -->
      <div
        class="th-stat-card bg-gradient-to-br from-th-warning/10 to-th-warning/5 border-th-warning/20"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-th-warning/5 to-transparent"
        ></div>
        <div class="relative p-2 h-full flex flex-col">
          <div class="flex items-center justify-between flex-1">
            <div class="flex-1">
              <div class="text-sm font-medium text-th-warning mb-1">
                Active Events
              </div>
              <div class="text-2xl font-bold text-foreground mb-1">
                {{ store.dashboardStats.activeEvents }}
              </div>
              <div class="text-xs text-foreground/60">
                {{ store.events.upcoming.length }} upcoming
              </div>
            </div>
            <div
              class="h-12 w-12 rounded-full flex items-center justify-center bg-th-warning/15"
            >
          ...
          <div v-if="nextEventTime" class="mt-3 flex items-center gap-1">
            ...
            <span class="text-xs text-th-warning font-medium"
              >Next: {{ nextEventTime }}</span
            >
          </div>
        </div>
      </div>
      ...
      <!-- Content Health Card -->
      <div
        class="th-stat-card bg-gradient-to-br from-th-success/10 to-th-success/5 border-th-success/20"
      >
        <div
          class="absolute inset-0 bg-gradient-to-br from-th-success/5 to-transparent"
        ></div>
        <div class="relative p-2 h-full flex flex-col">
          <div class="flex items-center justify-between flex-1">
            <div class="flex-1">
              <div class="text-sm font-medium text-th-success mb-1">
                Content Health
              </div>
              <div class="text-2xl font-bold text-foreground mb-1">
                {{ contentHealthScore }}%
            ...
            <div
              class="h-12 w-12 rounded-full flex items-center justify-center bg-th-success/15"
            >
    ...
    <section class="grid gap-6 lg:grid-cols-3">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="elev-card">
          ...
          <div v-if="store.topPerformers.length > 0" class="space-y-3">
            <div
              v-for="(guide, index) in store.topPerformers.slice(0, 6)"
              :key="guide.id"
            ...
            >
              ...
              <div class="text-right">
                <div class="text-sm font-bold text-th-primary">
                  {{ store.formatNumber(guide.views || 0) }} views
                </div>
                <div v-if="guide.rating" class="text-xs text-th-success">
          ...
          <div v-if="store.recentActivity.length > 0" class="space-y-2">
            ...
            >
              <div class="flex items-center gap-3">
                <component
                  :is="activity.icon"
                  :class="['h-4 w-4', activity.color]"
                />
                <span class="text-sm">{{ activity.message }}</span>
              </div>
              <span class="text-xs text-foreground/60">{{
                store.formatTimeAgo(activity.time)
              }}</span>
            </div>
          </div>
          <div v-else class="text-center py-8 text-foreground/60">
        ...
        <div v-if="store.events.upcoming.length > 0" class="elev-card">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div
              v-for="event in store.events.upcoming.slice(0, 4)"
              :key="event.id"
              ...
              <div class="space-y-1">
                <div class="flex justify-between text-xs">
                  ...
                  <span class="font-medium">{{
                    store.formatTimeAgo(event.startTime)
                  }}</span>
                </div>
                <div
      ...
      <div class="space-y-6">
        ...
        <div class="elev-card">
          ...
          <div class="grid grid-cols-2 gap-3">
            <button
              ...
              class="tool-tile hover:border-th-primary/50"
            >
            ...
            <button
              ...
              class="tool-tile hover:border-th-secondary/50"
            >
            ...
            <button
              @click="navigateTo('events')"
              class="tool-tile hover:border-th-warning/50"
            >
            ...
            <button
              ...
              class="tool-tile hover:border-th-success/50"
            >
        ...
        <div v-if="store.dashboardInsights.length > 0" class="elev-card">
          ...
          <div class="space-y-3">
            <div
              v-for="(insight, index) in store.dashboardInsights"
              :key="index"
            ...
            >
              <div
                :class="[
                  ...
                  getInsightIconBg(insight.color),
                ]"
              >
                <component
                  :is="getInsightIcon(insight.icon)"
        ...
        <div class="elev-card">
          ...
          <div class="space-y-3">
            <div
              v-for="health in store.systemHealth"
              :key="health.metric"
        ...
        <div class="elev-card">
          ...
          <div
            v-if="store.dashboardStats.recentGuides.length > 0"
            class="space-y-2"
          >
            <div
              v-for="guide in store.dashboardStats.recentGuides.slice(0, 5)"
              :key="guide.id"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-foreground/60">
                  {{ guide.category }} •
                  {{ store.formatTimeAgo(guide.lastUpdated) }}
                </div>
              </div>
              <div
                :class="[
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between">
              ...
              <span
                :class="[
                  'font-medium',
                  store.isCacheStale ? 'text-th-warning' : 'text-th-success',
                ]"
              >
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium"
                >{{
                  store.dashboardStats.totalGuides +
                  store.dashboardStats.totalHeroes
                }}
                items</span
              >
            </div>
...
const contentHealthScore = computed(() => {
  ...
  const upToDate = store.guides.list.filter((g) => {
    const daysSince =
      (Date.now() - (g.lastUpdated || 0)) / (1000 * 60 * 60 * 24);
    return daysSince <= 30;
  }).length;
