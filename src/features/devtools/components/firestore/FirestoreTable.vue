<template>
  <div class="bg-gray-900/80 shadow-md overflow-hidden">
    <div v-if="!selectedCollection" class="p-12 text-center text-gray-400">
      <div class="flex flex-col items-center gap-4">
        ...
        <p class="text-sm text-gray-500">
          Choose a collection to view documents
        </p>
      </div>
    </div>
    ...
    <div v-else-if="loading" class="p-12 text-center">
      <svg
        class="animate-spin h-12 w-12 text-indigo-500 mx-auto"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
      >
    ...
    <div v-else>
      <Table>
          ...
          <TableRow class="bg-gray-900/100 hover:bg-gray-900/80">
            ...
            <TableHead class="px-6 py-4 text-gray-400 font-semibold">
              ...
              >
                Doc ID
                <ChevronDownIcon
                  v-if="sortBy === 'id' && sortDirection === 'asc'"
                  class="h-4 w-4"
                />
                <ChevronUpIcon
                  v-if="sortBy === 'id' && sortDirection === 'desc'"
                  class="h-4 w-4"
                />
              </button>
            </TableHead>
            ...
            <TableHead class="px-6 py-4 min-w-36 text-gray-400 font-semibold">
              ...
              >
                Last Modified
                <ChevronDownIcon
                  v-if="sortBy === 'updatedAt' && sortDirection === 'asc'"
                  class="h-4 w-4"
                />
                <ChevronUpIcon
                  v-if="sortBy === 'updatedAt' && sortDirection === 'desc'"
                  class="h-4 w-4"
                />
              </button>
            </TableHead>

            <TableHead class="text-right px-6 py-4 text-gray-400 font-semibold"
              >Actions</TableHead
            >
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
          <TableRow
            v-for="doc in filteredData"
            :key="doc.id"
            class="hover:bg-gray-800 transition"
          >
            <TableCell class="font-mono pl-6 text-indigo-300">{{
              doc.id
            }}</TableCell>
            <TableCell class="text-gray-400 truncate max-w-xs">{{
              formatPreview(doc.data)
            }}</TableCell>
            <TableCell class="text-gray-400"
              >{{ formatDate(doc.data.updatedAt || doc.data.createdAt) }}
            </TableCell>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      ...
                      >
                        <EyeIcon
                          class="h-5 w-5 text-indigo-400 hover:text-indigo-300"
                        />
                      </button>
                    </TooltipTrigger>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      ...
                      >
                        <PencilIcon
                          class="h-5 w-5 text-blue-400 hover:text-blue-300"
                        />
                      </button>
                    </TooltipTrigger>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      ...
                      >
                        <TrashIcon
                          class="h-5 w-5 text-red-400 hover:text-red-300"
                        />
                      </button>
                    </TooltipTrigger>
