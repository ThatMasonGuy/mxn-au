<template>
  ...
  >
    ...
    <div
      class="flex flex-col sm:flex-row sm:items-center gap-4 text-sm text-gray-400"
    >
      <div class="flex items-center gap-2">
        <span class="text-gray-400">Mock Data:</span>

        <div class="relative inline-flex items-center cursor-not-allowed">
          <input type="checkbox" class="sr-only" checked disabled />
          <div
            class="w-10 h-5 bg-indigo-600 rounded-full shadow-inner transition"
          ></div>
          <div
            class="absolute left-1 top-1 w-3 h-3 bg-white rounded-full shadow transition"
          ></div>
        </div>

        <span class="text-indigo-400 font-medium">Enabled</span>
      </div>
