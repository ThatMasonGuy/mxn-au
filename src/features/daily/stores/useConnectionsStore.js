...
export const useConnectionsStore = defineStore("connections", {
  ...
  getters: {
    ...
    selected() {
      return this.todayState.selected || [];
    },
    foundGroups() {
      return this.todayState.foundGroups || [];
    },
    mistakes() {
      return this.todayState.mistakes || 0;
    },
    attempts() {
      return this.todayState.attempts || [];
    },
    status() {
      return this.todayState.status || "idle";
    },
    boardWords() {
      return this.todayState.boardWords || [];
    },

    remainingLives() {
      return this.MAX_MISTAKES - this.mistakes;
    },
    isComplete() {
      return this.status === "won" || this.status === "lost";
    },
    canSubmit() {
      return this.selected.length === this.GROUP_SIZE && !this.isComplete;
    },
  ...
  actions: {
    ...
    _syncWithDailyStore() {
      try {
        ...
        if (this.profile)
          dailyStore.updateGameStats?.("connections", this.profile);
        if (this.status && this.status !== "idle")
          dailyStore.updateGameStatus?.("connections", this.status);
    ...
    async _doLoad(preferServer = false) {
      ...
      try {
        ...
        if (
          this.groups &&
          (!this.days[date].boardWords ||
            this.days[date].boardWords.length === 0)
        ) {
          const allWords = [
            ...this.groups.easy,
            ...this.groups.medium,
    ...
    async checkGuess(selectedWords) {
      ...
      const previousAttempts = this.attempts.map((a) =>
        [...a].sort().join(","),
      );
      if (previousAttempts.includes(guess.join(","))) {
        return {
          result: "duplicate",
          message: "Already tried this combination",
        };
      }
