...
function sanitizePersisted(parsed) {
  const safe = {
    availableWords: Array.isArray(parsed?.availableWords)
      ? parsed.availableWords.slice(0, 200)
      : [],
    currentGame: isValidGame(parsed?.currentGame) ? parsed.currentGame : null,
    currentAnswer:
      typeof parsed?.currentAnswer === "string" ? parsed.currentAnswer : null,
    _v: PERSIST_VERSION,
  };
  return safe;
}
  ...
  actions: {
      ...
      try {
        ...
        if (this.profile) {
          dailyStore.wordleUnlimitedStats = {
            ...
            attemptsHistogram: this.profile.attemptsHistogram || [
              0, 0, 0, 0, 0, 0,
            ],
          };
        }
      ...
      try {
        ...
        if (Array.isArray(data?.words)) {
          ...
          if (this.currentGame?.word)
            this.playedWords.add(this.currentGame.word);
        }
      } catch (error) {
    ...
    async startNewGame({ force = false } = {}) {
      ...
      try {
        ...
        if (
          !Array.isArray(this.availableWords) ||
          this.availableWords.length < 5
        ) {
      ...
      try {
        ...
        if (mostRecentInProgress) {
          ...
          const rows = mostRecentInProgress.guesses.map((guess, i) => ({
            ...
            mask: mostRecentInProgress.masks[i]
              ? mostRecentInProgress.masks[i].split("")
              : ["B", "B", "B", "B", "B"],
          }));
