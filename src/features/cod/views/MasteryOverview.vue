      ...
      <div class="space-y-4">
        ...
        <div class="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <div
                class="w-12 h-12 rounded-lg bg-gradient-to-r from-yellow-600 to-amber-600 flex items-center justify-center"
              >
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">
                  Complete all base camos (9 Military + 3 Special)
                </p>
              </div>
            </div>
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">
                  Get Gold on all weapons in each category
                </p>
              </div>
            </div>
            <div class="text-right">
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                <div
                  ...
                  :class="
                    isCategoryComplete(type, category)
                      ? 'bg-cyan-500'
                      : 'bg-gray-600'
                  "
                ></div>
              ...
              <div class="flex items-center justify-between">
                <span class="text-sm font-bold text-white">
                  {{ getCategoryGoldCount(type, category) }}/{{
                    getCategoryTotalWeapons(category)
                  }}
                </span>
                <component
                  :is="
                    isCategoryComplete(type, category) ? CheckCircle2 : Circle
                  "
                  class="w-4 h-4"
                  :class="
                    isCategoryComplete(type, category)
        ...
        <div class="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <div
                class="w-12 h-12 rounded-lg bg-gradient-to-r from-purple-600 to-violet-600 flex items-center justify-center"
              >
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">
                  Complete challenges on 30 weapons total
                </p>
              </div>
            </div>
            <div class="text-right">
              <div class="text-3xl font-bold text-purple-400">
                {{ getMasteryProgress(type, 3).completed
          ...
          <div class="h-3 bg-gray-700 rounded-full overflow-hidden">
            <div
              ...
              :style="{
                width: `${Math.min(100, (getMasteryProgress(type, 3).completed / 30) * 100)}%`,
              }"
            ></div>
          </div>
        </div>
        ...
        <div class="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <div
                class="w-12 h-12 rounded-lg bg-gradient-to-r from-pink-600 to-rose-600 flex items-center justify-center"
              >
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">
                  Auto-unlocks after 30 Dark Spine completions
                </p>
              </div>
            </div>
            <div class="text-right">
              <div
                ...
                :class="
                  getMasteryProgress(type, 3).completed >= 30
                    ? 'text-pink-400'
                    : 'text-gray-600'
                "
              >
              ...
              <div class="text-sm text-gray-400">
                {{
                  getMasteryProgress(type, 3).completed >= 30
                    ? "Complete"
                    : `${30 - getMasteryProgress(type, 3).completed} to go`
                }}
              </div>
            </div>
          </div>
          <div class="h-3 bg-gray-700 rounded-full overflow-hidden">
            <div
              ...
              :style="{
                width:
                  getMasteryProgress(type, 3).completed >= 30 ? '100%' : '0%',
              }"
            ></div>
          </div>
        </div>
...
const getMasteryProgress = (type, level) => {
  ...
  const completed = camoStore.weapons.filter(
    (w) => w[typeKey].mastery >= level,
  ).length;
...
const getCategoryGoldCount = (type, category) => {
  const typeKey = type.toLowerCase();
  return camoStore.weapons.filter(
    (w) => w.category === category && w[typeKey].mastery >= 1,
  ).length;
};
...
const isCategoryComplete = (type, category) => {
  const typeKey = type.toLowerCase();
  const categoryWeapons = camoStore.weapons.filter(
