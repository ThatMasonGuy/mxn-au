  ...
  >
    <div class="w-full max-w-6xl">
        ...
        >
          ...
          <template v-if="tokenExpired">
            <div class="flex flex-col items-center text-center">
              ...
              <p class="text-gray-300 mb-8 leading-relaxed text-lg">
                Your Bungie session has expired. Please re-authenticate to
                continue tracking your challenge progress and unlock AI-powered
                optimization.
              </p>

              <!-- Enhanced Button -->
              <button
                @click="handleLinkBungie"
                :disabled="loading"
              ...
              >
                ...
                ></div>
                <component
                  :is="loading ? RotateCw : Link2Off"
                  class="w-6 h-6 relative z-10"
                  :class="{ 'animate-spin': loading }"
                />
                <span class="relative z-10">{{
                  loading ? "Reconnecting…" : "Re-Link Account"
                }}</span>
              </button>
            </div>
          </template>
          ...
          <template v-else>
            <div class="flex flex-col items-center text-center">
              ...
              <p class="text-gray-300 mb-8 leading-relaxed text-lg">
                Link your Bungie account to unlock real-time challenge tracking,
                progress analytics, and AI-powered optimization strategies to
                maximize your seasonal rewards.
              </p>
              ...
              >
                ...
                ></div>
                <component
                  :is="loading ? RotateCw : Link2"
                  class="w-6 h-6 relative z-10"
                  :class="{ 'animate-spin': loading }"
                />
                <span class="relative z-10">{{
                  loading ? "Connecting…" : "Connect Bungie Account"
                }}</span>
              </button>
            </div>
          </template>
      ...
      <div v-else class="space-y-6">
        <!-- Profile & Controls -->
        <div
          class="bg-black/30 backdrop-blur-sm rounded-3xl p-6 border border-purple-500/20 shadow-2xl"
        >
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="text-sm text-gray-400">
                  {{ getPlatform }} • Week {{ weeklyInfo.currentWeek }} •
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            >
              ...
              <div class="text-xs text-gray-500 mt-1">
                {{
                  isNaN(activeCompletionRate)
                    ? "0.0"
                    : activeCompletionRate.toFixed(1)
                }}% active rate
              </div>
            </div>
            <div
            ...
            >
              ...
              <div class="text-xs text-gray-500 mt-1">
                {{ challengesArray.filter((c) => c.active).length }} available
              </div>
            </div>
            <div
            ...
            >
              ...
              <div class="text-xs text-gray-500 mt-1">
                {{ inactiveChallenges.length }} locked
              </div>
            </div>
            <div
            ...
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                ...
                <span class="text-violet-300 font-semibold"
                  >Season Week {{ weeklyInfo.currentWeek }}</span
                >
              </div>
              <div class="text-sm text-gray-400">
                {{ weeklyInfo.availableChallenges }} of
                {{ challengesArray.length }} challenges available
              </div>
            </div>
                ...
                :style="{
                  width:
                    challengesArray.length > 0
                      ? `${(weeklyInfo.availableChallenges / challengesArray.length) * 100}%`
                      : '0%',
                }"
              ></div>
            </div>
            <div class="text-xs text-gray-500 mt-2">
              <span v-if="weeklyInfo.currentWeekChallenges > 0"
                >{{ weeklyInfo.currentWeekChallenges }} new challenges this week
                •
              </span>
              {{
                Math.max(
                  0,
                  challengesArray.length - weeklyInfo.availableChallenges,
                )
              }}
              challenges will unlock in future weeks
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <MultiSelect v-model="filterStatus">
              <MultiSelectTrigger
                class="w-full bg-gray-800/50 text-white border-gray-600"
              ...
              >
                <MultiSelectItem value="All Challenges"
                  >All Challenges</MultiSelectItem
                >
                <MultiSelectItem value="Active Only"
                  >Active Only</MultiSelectItem
                >
                <MultiSelectItem value="Locked Only"
                  >Locked Only</MultiSelectItem
                >
                <MultiSelectItem value="Completed">Completed</MultiSelectItem>
                <MultiSelectItem value="Incomplete">Incomplete</MultiSelectItem>
                <MultiSelectItem value="Near Complete"
                  >Near Complete (80%+)</MultiSelectItem
                >
              </MultiSelectContent>
            </MultiSelect>
            ...
            <Select v-model="sortBy">
              ...
              <SelectContent class="bg-gray-800 text-white border-gray-600">
                <SelectItem value="availability"
                  >Sort by Availability</SelectItem
                >
        ...
        >
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-gray-400">
                  Smart recommendations for active challenges (Week
            ...
            <button
              ...
              :disabled="
                suggestLoading ||
                loading ||
                activeIncompleteChallenges.length === 0
              "
            ...
            >
              <component
                :is="suggestLoading ? RotateCw : Lightbulb"
                class="w-5 h-5"
                :class="{ 'animate-spin': suggestLoading }"
              />
              <span v-if="activeIncompleteChallenges.length === 0"
                >No Active Challenges</span
              >
              <span v-else>{{
                suggestLoading ? "Analyzing..." : "Generate Plan"
              }}</span>
            </button>
          </div>
          ...
          <div
            v-if="aiSuggestion"
            class="bg-black/30 rounded-2xl p-6 border border-pink-500/20"
          >
            <div
            ...
            <!-- AI Metadata -->
            <div
              v-if="aiMetadata.challengeCount > 0"
            ...
            >
              ...
              >
                <div class="flex items-center gap-1">
                  ...
                  <span
                    >{{ aiMetadata.challengeCount }} active challenges
                    analyzed</span
                  >
                </div>
          ...
          <div v-else class="text-center py-8">
            ...
            <p
              v-if="activeIncompleteChallenges.length === 0"
              class="text-gray-400 text-lg"
            >
              {{
                activeChallengesLocal.length === 0
                  ? "No challenges available yet this week"
                  : "All active challenges completed!"
              }}
            </p>
            <p v-else class="text-gray-400 text-lg">
              Click "Generate Plan" to get AI-powered optimization suggestions
            </p>
        ...
        >
            ...
            <div class="flex items-center gap-2 text-sm text-gray-400">
              ...
              <div class="w-32 h-2 bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  :style="{
                    width: `${isNaN(activeCompletionRate) ? 0 : activeCompletionRate}%`,
                  }"
                ></div>
              </div>
            </div>
          ...
          <div v-else-if="displayedChallenges.length" class="space-y-4">
            <div
              ...
              :class="{
                // Active challenges
                'bg-gradient-to-r from-gray-800/50 to-gray-700/50 border-gray-600/30 hover:border-purple-500/50 hover:shadow-purple-500/10':
                  challenge.active,
                // Locked challenges
                'bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/30 hover:border-gray-600/50 opacity-60':
                  !challenge.active,
              }"
            >
                  ...
                  <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                      <h3
                        ...
                        :class="{
                          'text-gray-100 group-hover:text-purple-200':
                            challenge.active,
                  ...
                  <div
                    ...
                    :class="{
                      ...
                      'bg-gradient-to-r from-green-500 to-emerald-500':
                        challenge.active && challenge.complete,
                      'bg-gradient-to-r from-yellow-500 to-orange-500':
                        challenge.active &&
                        !challenge.complete &&
                        challenge.progressPercent >= 80,
                      'bg-gradient-to-r from-blue-500 to-cyan-500':
                        challenge.active &&
                        !challenge.complete &&
                        challenge.progressPercent >= 50 &&
                        challenge.progressPercent < 80,
            ...
            <!-- Load More Buttons -->
            <div
              v-if="filteredChallenges.length > displayCount"
              class="flex justify-center gap-4 pt-6"
            >
              <button
                @click="loadMore"
        ...
        <p
          v-if="error"
          class="text-red-400 text-center bg-red-900/20 px-4 py-3 rounded-lg"
        >
          {{ error }}
        </p>
      </div>
      ...
      <div class="text-center mt-16 pt-8 border-t border-gray-700/30">
        <p class="text-sm text-gray-500">
          Built by <span class="text-violet-300 font-medium">Mason</span> •
          Powered by <span class="text-cyan-300 font-medium">OpenAI</span>
        </p>
      </div>
    </div>
  </div>
...
import { useDestinyStore } from "@/features/destiny/stores/useDestinyStore";
...
const challengesArray = computed(() => {
  ...
  const list = Array.isArray(challenges.value)
    ? challenges.value
    : Object.values(challenges.value);
  return list.map((chal) => {
    const obj =
      Array.isArray(chal.objectives) && chal.objectives.length > 0
        ? chal.objectives[0]
        : {};
...
const activeInProgressCount = computed(() => {
  return challengesArray.value.filter(
    (c) => c.active && !c.complete && c.progress > 0,
  ).length;
});

const activeNearCompletionCount = computed(() => {
  return challengesArray.value.filter(
    (c) => c.active && !c.complete && c.progressPercent >= 80,
  ).length;
});
...
const activeAvgProgress = computed(() => {
  ...
  const total = activeChallengesArray.reduce(
...
const activeIncompleteChallenges = computed(() => {
  return incompleteChallenges.value.filter(
...
watch(
  ...
  (newValue, oldValue) => {
    if (
      newValue.includes("All Challenges") &&
      !oldValue.includes("All Challenges")
    ) {
...
const filteredChallenges = computed(() => {
  ...
  if (
    ...
    !filterStatus.value.includes("All Challenges")
  ) {
