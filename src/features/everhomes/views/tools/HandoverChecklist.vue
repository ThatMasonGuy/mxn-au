<!-- src/features/everhomes/views/tools/HandoverChecklist.vue -->
  ...
  <LayoutComponent :header="true" :footer="true">
    ...
    <div class="bg-slate-900 border-b border-slate-800">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 h-10 flex items-center gap-3">
        <span class="text-xs font-black tracking-wider text-teal-400 uppercase"
          >Everhomes</span
        >
        <span class="text-slate-700">|</span>
        <span class="text-xs font-semibold text-slate-400"
          >Handover Review</span
        >
      </div>
    </div>
    ...
    <div class="">
      <div
        class="bg-gradient-to-r from-teal-700 to-emerald-600 px-4 sm:px-6 lg:px-8 pt-8 pb-10"
      >
        <div class="max-w-4xl mx-auto">
          <h1
            class="text-2xl sm:text-3xl font-black text-white tracking-tight uppercase"
          >
            Handover / Annual Review
          </h1>
          <p class="text-teal-200 text-sm mt-1 font-medium">
            SDA Dwelling Suitability Review — select the sections applicable to
            this property.
          </p>
        </div>
      </div>
        ...
        >
          ...
          >
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label
                  class="block text-[0.65rem] font-black text-teal-600 uppercase tracking-widest mb-1"
          ...
          <div class="px-5 py-6 space-y-6">
            ...
            <div>
              ...
              <p class="text-[0.68rem] text-slate-400 font-medium mb-3">
                Filters checklist items to those applicable for this property
              </p>
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button
                  v-for="pt in PROPERTY_TYPES"
                  :key="pt.key"
                  @click="propertyType = pt.key"
                ...
                >
                  ...
                  <span
                    class="text-[0.65rem] font-medium leading-tight opacity-80"
                    >{{ pt.label }}</span
                  >
                  <div class="flex flex-wrap gap-1 mt-0.5">
                    <span
                      v-for="cat in pt.includes"
                      :key="cat"
                      class="text-[0.55rem] font-black px-1.5 py-0.5 rounded-md uppercase"
                      :style="sdaChipStyle(cat)"
                      >{{ cat }}</span
                    >
                  </div>
                </button>
              ...
              <p
                v-if="setupErrors.propertyType"
                class="text-rose-500 text-[0.65rem] font-bold mt-2"
              >
            ...
            <div>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <button
                  v-for="section in visibleOptionalSections"
                  :key="section.key"
                  ...
                  :class="
                    selectedOptional.has(section.key)
                ...
                >
                  ...
                  >
                    <Check
                      v-if="selectedOptional.has(section.key)"
    ...
    <div data-checklist class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
      <div
        v-if="checklistSections.length === 0"
      ...
      >
        ...
        <p class="text-sm font-semibold text-slate-400">
          Configure your review above<br /><span
            class="text-slate-500 font-normal"
            >then hit Start Handover Review.</span
          >
        </p>
      </div>
      ...
      <template v-else>
        ...
        >
          ...
          <p class="text-xs text-amber-300/90 font-medium leading-relaxed">
            ...
            report. Use the Marketing Photos section at the bottom for
            uncompressed hero shots.
          </p>
        </div>
        ...
        <div class="mb-5">
          ...
          <div class="flex items-center justify-between">
            <span class="text-xs text-slate-500 font-medium"
              >{{ inspectedCount }} / {{ checklistSections.length }} sections
              done</span
            >
            <span
          ...
          >
            ...
            >
              <div
                ...
                :style="{
                  background:
                    SECTION_META[section.type]?.gradient ??
                    SECTION_META.general.gradient,
                }"
              >
                <component
                  :is="SECTION_META[section.type]?.icon ?? ClipboardCheck"
                  class="w-4 h-4"
                />
              </div>
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs mt-0.5">
                  <span class="text-slate-500"
                    >{{
                      checklistData[section.id]?.photos?.length ?? 0
                    }}
                    photo{{
                      (checklistData[section.id]?.photos?.length ?? 0) !== 1
                        ? "s"
                        : ""
                    }}
                    ·
                  ...
                  <span
                    :style="{
                      color: statusColour(checklistData[section.id]?.status),
                    }"
              ...
              <div
                ...
                :style="{
                  background: statusColour(checklistData[section.id]?.status),
                }"
              ></div>
              <ChevronDown
            ...
            >
              <div
                v-if="openSections.has(idx) && checklistData[section.id]"
              ...
              >
                  ...
                  <template v-if="sectionIsAllNA(section.id)">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <button
                        ...
                        :class="
                          checklistData[section.id].status === s.value
                            ? s.activeClass
                      ...
                      <component
                        :is="
                          STATUS_OPTIONS.find(
                            (s) =>
                              s.value ===
                              (checklistData[section.id].status || 'unchecked'),
                          )?.icon ?? MinusCircle
                        "
                        ...
                        <template v-for="item in group.items" :key="item.id">
                          ...
                          >
                            ...
                            <input
                              ...
                              :value="
                                checklistData[section.id].inputs?.[item.id] ??
                                ''
                              "
                              @input="
                                setItemInput(
                          ...
                          <div
                            ...
                            :class="
                              itemRowClass(
                                checklistData[section.id].items?.[item.id],
                              )
                            "
                          >
                            <div
                            ...
                            <div class="flex items-center gap-0.5 shrink-0">
                              <button
                                ...
                                :key="s.value"
                                @click="
                                  setItemStatus(section.id, item.id, s.value)
                                "
                                :title="s.label"
                ...
                <div>
                  ...
                  >
                  <textarea
                    v-model="checklistData[section.id].notes"
                    rows="3"
                ...
                <div>
                  ...
                  <div
                    v-if="checklistData[section.id].photos.length"
                    class="grid grid-cols-3 sm:grid-cols-4 gap-2"
                  >
                    <div
                      v-for="(photo, pIdx) in checklistData[section.id].photos"
                      :key="pIdx"
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                          <button
                            @click.stop="retryPhotoUpload(section.id, pIdx)"
                  ...
                  <div
                    v-if="hasSkippedPhotos(section.id)"
        ...
        >
          <div class="px-5 py-4 border-b border-teal-500/15 bg-teal-500/5">
            ...
            <p class="text-[0.65rem] text-slate-400 leading-relaxed">
              ...
              <span class="font-bold text-teal-300">uncompressed</span> — not
        ...
        <div class="mt-6 pt-6 border-t border-slate-700/50">
          ...
          <p class="text-center text-xs text-slate-600 font-medium mt-2">
            {{
              uninspectedCount > 0
                ? `${uninspectedCount} section${uninspectedCount !== 1 ? "s" : ""} still
                        unchecked`
                  ...
                  <div class="space-y-1.5">
                    ...
                    >
                      <div
                        ...
                        :style="{
                          background:
                            SECTION_META[sec.type]?.gradient ??
                            SECTION_META.general.gradient,
                        }"
                      >
                        <component
                          :is="SECTION_META[sec.type]?.icon ?? ClipboardCheck"
                          class="w-3.5 h-3.5"
                        />
                      </div>
                      <div class="flex-1 min-w-0">
                        ...
                        <p
                          ...
                          :style="{ color: statusColour(sec.status) }"
                        >
                  ...
                  <div class="px-4 py-4 space-y-3">
                    ...
                    >
                      ...
                      >
                        ...
                        >, confirm that this handover review was completed on
                        <span class="font-bold text-white">{{
        ...
        >
          ...
          >
            ...
            <p class="text-slate-400 text-sm mb-5">
              All review data and photos will be permanently removed.
            </p>
            <div class="flex gap-3">
              <button
                @click="confirmClear.open = false"
              ...
              <template v-else>
                ...
                >
                  ...
                  >
                    ...
                    />
                    <span
                      class="text-xs font-bold text-slate-400 group-hover:text-teal-400 transition-colors"
                      >Take Photo</span
                    >
                  </button>
                  <button
...
function ensureEntry(id, type) {
  if (!checklistData[id]) {
    const items = {};
    getAllItemIds(type).forEach((itemId) => {
...
const visibleOptionalSections = computed(() =>
  OPTIONAL_SECTIONS.filter(
    (s) => s.key !== "breakout" || propertyType.value === "R",
  ),
);
...
function itemApplicable(item) {
  ...
  const allowed =
    PROPERTY_TYPES.find((p) => p.key === propertyType.value)?.includes ?? [];
...
const progressPercent = computed(() => {
  ...
  return Math.round(
    (inspectedCount.value / checklistSections.value.length) * 100,
  );
});
...
function openPhotoModal(idx) {
  Object.assign(photoModal, {
    open: true,
    sectionIdx: idx,
    sectionLabel: checklistSections.value[idx]?.label ?? "",
    preview: null,
    rawFile: null,
    caption: "",
    queue: [],
...
function hasSkippedPhotos(sectionId) {
  return (
    checklistData[sectionId]?.photos?.some(
      (p) => p.uploadStatus === "skipped",
    ) ?? false
  );
}

const hasAnyUploading = computed(() =>
  Object.values(checklistData).some((d) =>
...
function saveCaptionEdit() {
  const photo =
    checklistData[captionEdit.sectionId]?.photos?.[captionEdit.photoIdx];
...
async function flushAndCheck() {
  ...
  try {
    ...
    if (!data) {
      submitModal.loading = false;
      submitModal.error =
        "Report record not found on the server. Your data is safe — please try submitting again.";
...
function loadFromCache() {
  try {
    ...
    if (
      Array.isArray(data.checklistSections) &&
      data.checklistSections.length
    ) {
