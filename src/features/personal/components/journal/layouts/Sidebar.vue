  ...
  >
    ...
    <div class="flex-1 overflow-y-auto">
      <div v-if="isSidebarOpen" class="p-3">
        <SidebarGroup
          v-for="(group, dateKey) in groupedChats"
          :key="dateKey"
          :group="group"
          :formatted-date="formatDate(dateKey)"
          ...
          @confirm-rename="
            ($eventChat, newName) =>
              $emit('confirm-rename', $eventChat, newName)
          "
      ...
      <div v-else class="py-3 flex flex-col items-center gap-3">
        <template
          v-for="(group, dateKey) in groupedChats"
          :key="`condensed-${dateKey}`"
        >
          <div
            class="text-xs font-medium text-zinc-500 text-center mb-2 w-full"
          >
