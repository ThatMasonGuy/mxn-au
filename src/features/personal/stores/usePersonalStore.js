// @/features/personal/stores/usePersonalStore.js
...
export const usePersonalStore = defineStore("personalStore", () => {
  ...
  watchEffect(() => {
    ...
    if (uid.value) {
      ensureGoblinTrackerDocExists(uid.value).catch((error) => {
  ...
  async function loadMoreLogs() {
    if (!goblinCollection.value || isLoadingMore.value || !hasMoreLogs.value)
      return;
  ...
  const todaySummary = computed(() => {
    ...
    return (
      dailySummaries.value.find((s) => s.date === today) || {
        date: today,
        dateObj: new Date(),
        logs: [],
        totals: {
  ...
  const todayProgress = computed(() => {
    ...
    return {
      calories: {
        ...
        percentage: goals.calories
          ? Math.round((today.calories / goals.calories) * 100)
          : 0,
      ...
      protein: {
        ...
        percentage: goals.protein
          ? Math.round((today.protein / goals.protein) * 100)
          : 0,
      ...
      carbs: {
        ...
        percentage: goals.carbs
          ? Math.round((today.carbs / goals.carbs) * 100)
          : 0,
      ...
      water: {
        ...
        percentage: goals.water
          ? Math.round((today.water / goals.water) * 100)
          : 0,
      ...
      sodium: {
        ...
        percentage: goals.sodium
          ? Math.round((today.sodium / goals.sodium) * 100)
          : 0,
  ...
  async function addGoblinLog(log) {
    ...
    if (
      log.item === "Dare Double Espresso" ||
      log.item === "Dare Intense Espresso"
    ) {
      if (log.variant === "750ml") {
        await updateDoc(trackerRef, {
          dareCountBig: increment(log.servings || 1),
        });
      } else if (log.variant === "500ml") {
        await updateDoc(trackerRef, {
          dareCountSmall: increment(log.servings || 1),
        });
      }
    }
  }
