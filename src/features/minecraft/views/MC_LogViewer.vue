  ...
  <div class="p-6 max-w-full h-screen flex flex-col">
          ...
          <div class="flex flex-col gap-4">
            <div>
              ...
              <div class="flex items-center gap-2">
                <input
                  v-model.number="exportLineInput"
                  type="number"
                  min="1"
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <label class="flex items-center gap-1">
                  <input type="checkbox" value="chat" v-model="exportFilters" />
                  Chat
                </label>
                <label class="flex items-center gap-1">
                  <input type="checkbox" value="join" v-model="exportFilters" />
                  Join/Leave
                </label>
                <label class="flex items-center gap-1">
                  <input
    ...
    >
      ...
      <div
        ...
        :key="index"
        class="border-b border-gray-700 pb-1"
        :class="getLogColor(line)"
        :style="getLogStyle(line)"
      >
        <span class="log-line">
          <template v-if="getSourceBadge(line)">
            <span
              :class="getSourceBadge(line).class"
    ...
    <transition name="fade">
      <button
        v-show="!autoScroll"
        @click="jumpToLive"
        class="fixed bottom-8 right-12 w-12 h-12 flex items-center justify-center bg-white/10 backdrop-blur text-white rounded-full shadow-md hover:bg-white/20 transition-transform duration-300 hover:scale-110 z-50"
        title="Jump to Live"
      >
  ...
  try {
    ...
    if (res.data && Array.isArray(res.data.logs)) {
      ...
      if (logs.value.length === 0) {
        ...
        const lastIndex = incomingLogs.findIndex(
          (line) => line === lastLog.value,
        );
        const newLines =
          lastIndex === -1
...
const getSourceBadge = (line) => {
  if (line.includes("[DiscordSRV]"))
    return { label: "DISCORD", class: "bg-purple-600 text-white" };
  if (line.includes("Async Chat Thread"))
    return { label: "MINECRAFT", class: "bg-green-600 text-white" };
  if (/issued server command/i.test(line))
    return { label: "COMMAND", class: "bg-orange-600 text-white" };
  return null;
};
...
const highlightPlayer = (line) => {
  return line
    ...
    .replace(
      /\[DiscordSRV\]/g,
      '<span class="text-purple-400">[DiscordSRV]</span>',
    );
};
...
const filteredLogs = computed(() => {
  return logs.value.filter((line) => {
    ...
    if (selectedType.value === "chat")
      return (
        termMatch &&
        (/\[Async Chat Thread/.test(line) || /\[DiscordSRV\] Chat:/.test(line))
      );
    if (selectedType.value === "join")
      return termMatch && /joined the game|left the game/i.test(line);
    if (selectedType.value === "error")
      return termMatch && /\b(error|exception|failed)\b/i.test(line);
    if (selectedType.value === "command")
      return termMatch && /issued server command/i.test(line);
    return termMatch;
  });
});
    ...
    if (exportFilters.value.length) {
      exportData = exportData.filter((line) => {
        return exportFilters.value.some((type) => {
          if (type === "chat")
            return (
              /\[Async Chat Thread/.test(line) ||
              /\[DiscordSRV\] Chat:/.test(line)
            );
          if (type === "join")
            return /joined the game|left the game/i.test(line);
          if (type === "error")
