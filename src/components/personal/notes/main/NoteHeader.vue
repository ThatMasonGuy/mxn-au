    ...
    <div class="border-b border-zinc-200 dark:border-zinc-800 p-4">
        <div class="flex items-center justify-between">
            ...
            <div v-if="selectedNote">
                <h2 class="font-medium">{{ selectedNote.title || 'Untitled Note' }}</h2>
                <div class="text-xs text-zinc-500">
                    {{ formattedTime(selectedNote.updatedAt?.toMillis?.() || selectedNote.updatedAt || Date.now()) }}
                </div>
            </div>
            <div v-else>
                <h2 class="font-medium">No note selected</h2>
