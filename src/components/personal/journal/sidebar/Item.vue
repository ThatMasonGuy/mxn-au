<template>
    <div class="relative group">
        <div @click="selectChatItem"
            ...
            tabindex="0" @keydown.enter.prevent="selectChatItem" @contextmenu.prevent="openContextMenu">
            <div
                ...
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)"
                    class="h-5 w-5 text-indigo-500 dark:text-indigo-300" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
            ...
            <button
                ...
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" class="h-5 w-5" fill="none"
        ...
        <div v-if="!isSidebarOpen" @click="selectChatItem"
...
const props = defineProps({
    ...
    renamingChatIdGlobal: [String, null],
...
const isRenaming = computed(() => props.renamingChatIdGlobal === props.chat.id);
const isContextMenuOpen = computed(() => props.isContextMenuOpenGlobal === props.chat.id);

watch(() => props.chat.title, (newTitle) => {
    if (isRenaming.value) {
        localRenameInput.value = newTitle;
    }
});

const selectChatItem = () => {
    emit('select-chat', props.chat);
};

const openContextMenu = () => {
    emit('open-menu', props.chat.id);
};
...
const startRenameItem = () => {
    localRenameInput.value = props.chat.title;
    emit('start-rename', props.chat);
};

const confirmRenameItem = () => {
    if (localRenameInput.value.trim()) {
        emit('confirm-rename', props.chat, localRenameInput.value.trim());
    }
};
